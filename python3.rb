...
class Python3 < Formula
  ...
  skip_clean 'bin/pip3', 'bin/pip-3.2'
  skip_clean 'bin/easy_install3', 'bin/easy_install-3.2'
  ...
  def lib_cellar
    prefix / (if OS.mac? then "Frameworks/Python.framework/Versions/3.2" end) /
      "lib/python3.2"
  end
  ...
  def post_install
    ...
    # minor updates, such as going from 3.2.0 to 3.2.1:

    # Create a site-packages in HOMEBREW_PREFIX/lib/python3.2/site-packages
  ...
  def sitecustomize
    <<-EOF.undent
      ...
      if sys.version_info[0] != 3:
          ...
          opt_executable = '#{opt_bin}/python3.2'
          if os.path.commonprefix([os.path.realpath(e) for e in [opt_executable, sys.executable]]).startswith('#{rack}'):
              ...
              try:
                  ...
                  build_time_vars['LINKFORSHARED'] = '-u _PyMac_Error #{opt_prefix}/Frameworks/Python.framework/Versions/3.2/Python'
              except:
  ...
  test do
    ...
    system "#{bin}/python3", "-c", "import Tkinter; root = Tkinter.Tk()"
  end
end
